CREATE DATABASE FMU_PROJETO_WEB_TESTE;

USE FMU_PROJETO_WEB_TESTE;
...
DELIMITER $ CREATE FUNCTION FUNC_VALOR_AGENDAMENTO(ID_AGENDAMENTO INT) RETURNS FLOAT BEGIN RETURN(
	SELECT
		SUM(s.VALOR)
	FROM
		TB_SERVICO s
		INNER JOIN TB_SERVICO_AGENDAMENTO a ON a.ID_SERVICO = s.ID_SERVICO
	WHERE
		a.ID_AGENDAMENTO = ID_AGENDAMENTO
);
